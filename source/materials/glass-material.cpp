...
std::unique_ptr<Shader> GlassMaterial::createShader(const SurfacePoint& point, const Vector3& wo) const {
	float IORin;
	float IORout;
	...
	return std::make_unique<MFRefractionShader>(point.shadingNormal, point.tangent, point.bitangent,
		color, roughness, IORin, IORout);
}
