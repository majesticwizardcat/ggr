...
Shader* GlassMaterial::createShader(const SurfacePoint& point, const Vector3& wo, StackAllocator& alloc) const {
	float IORin;
	float IORout;
	...
	if (util::equals(roughness, 0.0f)) {
		return alloc.construct<RefractionShader>(RefractionShader(point.shadingNormal, point.tangent,
			point.bitangent, color, IORin, IORout));
	}
	return alloc.construct<MFRefractionShader>(MFRefractionShader(point.shadingNormal, point.tangent,
