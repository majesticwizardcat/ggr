...
std::unique_ptr<Shader> BlendMaterial::createShader(
	...
	std::unique_ptr<Shader> difShader = 
	...
	if (util::equals(roughness, 0.0f)) {
		glossShader = std::make_unique<SpecularShader>(
				point.shadingNormal, point.tangent, 
				point.bitangent, glossy);
		// Return in here?
	}
	else {
		glossShader = std::make_unique<MFReflectionShader>(
				point.shadingNormal, point.tangent, 
